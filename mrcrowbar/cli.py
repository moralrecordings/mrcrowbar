...
ARGS_COMMON: ArgsType = {
    ...
    ("--plain", "-p"): dict(
        dest="colour", action="store_false", help="Disable ANSI colour formatting"
    ),
    ("--version", "-V"): dict( action="version", version=f"%(prog)s {__version__}" ),
}
...
ARGS_DUMP: ArgsType = {
    ...
    ("--no-offsets", "-O"): dict(
        dest="show_offsets", action="store_false", help="Don't render line offsets"
    ),
    ("--no-glyphs", "-G"): dict(
        dest="show_glyphs", action="store_false", help="Don't render the glyph map"
    ),
    **ARGS_RANGE,
    **ARGS_COMMON,
}
...
ARGS_DIFF: ArgsType = {
    ...
    ("--all", "-a"): dict(
        dest="show_all", action="store_true", help="Show all lines"
    ),
    **ARGS_RANGE,
    **ARGS_COMMON,
}
...
ARGS_FIND: ArgsType = {
    ...
    ("--overlap", "-o"): dict(
        dest="overlap", action="store_true", help="Return overlapping matches"
    ),
    ("--ignore-case", "-i"): dict(
        dest="ignore_case",
        action="store_true",
...
mrcdump_parser = lambda: get_parser(
    args=ARGS_DUMP,
    description="Examine the contents of a file as hexadecimal.",
)
mrcdiff_parser = lambda: get_parser(
    args=ARGS_DIFF, description="Compare the contents of two files as hexadecimal."
)
mrchist_parser = lambda: get_parser(
    args=ARGS_HIST, description="Display the contents of a file as a histogram map."
)
mrcpix_parser = lambda: get_parser(
    args=ARGS_PIX, description="Display the contents of a file as a 256 colour image."
)
mrcgrep_parser = lambda: get_parser(
    args=ARGS_GREP,
    description="Display the contents of a file that matches a regular expression pattern.",
    epilog=EPILOG_GREP,
)
mrcfind_parser = lambda: get_parser(
    args=ARGS_FIND,
    description="Display the contents of a file that matches a string, checking against multiple encodings.",
)
...
def mrcdiff():
    ...
    for source1_fn, source2_fn in sources:
        with open( source1_fn, "rb" ) as source1_f, open(
            source2_fn, "rb"
        ) as source2_f:
            if multi:
