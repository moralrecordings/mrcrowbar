...
class BoppinCompressor( mrc.Transform ):
    def import_data( self, buffer, parent=None ):
        ...
        if len( result.payload ) != size_raw:
            logger.warning(
                f"{self}: Was expecting a decompressed size of {size_raw}, got {len( result.payload )}!"
            )
...
class Resource( mrc.Block ):
    files = mrc.BlockField(
        FileLookup,
        0x00,
        stride=0x08,
        stream=True,
        stop_check=resource_stop_check,
        fill=b"\xff\xff\xff\xff\x00\x00\x00\x00",
    )
    files_raw = mrc.Bytes( mrc.EndOffset( "files" ) )

    def __init__( self, *args, **kwargs ):
        self.files_store = mrc.Store(
            parent=self,
            source=mrc.Ref( "files_raw" ),
            base_offset=mrc.EndOffset( "files", neg=True ),
        )
