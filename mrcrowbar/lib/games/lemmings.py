    ...
    def import_data( self, buffer, parent=None ):
        ...
        def copy_prev_data( blocklen, offset_size, state ):
            ...
            for i in range( blocklen ):
                state["dptr"] -= 1
                state["ddata"][state["dptr"]] = state["ddata"][
                    state["dptr"] + offset + 1
                ]
            return
        ...
        return mrc.TransformResult(
            payload=bytes( state["ddata"] ), end_offset=pointer
        )
...
class SpecialCompressor( mrc.Transform ):
    ...
    def import_data( self, buffer, parent=None ):
        ...
        return mrc.TransformResult(
            payload=bytes( itertools.chain( *unpack ) ), end_offset=i
        )
...
class GroundDAT( mrc.Block ):
    ...
    interactive_info = mrc.BlockField(
        InteractiveInfo, 0x0000, count=16, fill=b"\x00" * 28
    )
    ...
    def palette( self ):
        if not hasattr( self, "_palette" ):
            self._palette = (
                [img.Transparent()]
                + self.palette_vga_standard[1:]
                + self.palette_vga_custom
            )
...
class VgagrDAT( mrc.Block ):
    stores = mrc.BlockField(
        VgagrStore, 0x0000, stream=True, transform=DATCompressor()
    )
...
AnimField = lambda offset, width, height, bpp, frame_count: mrc.BlockField(
    Anim,
    offset,
    block_kwargs={
        "width": width,
...
class Loader( mrc.Loader ):
    ...
    def __init__( self ):
        super().__init__(
            self._LEMMINGS_FILE_CLASS_MAP, dependency_list=self._LEMMINGS_DEPS
        )
