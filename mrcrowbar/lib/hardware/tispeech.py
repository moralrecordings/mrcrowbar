    ...
    def dump_stream( cls, frames: Sequence[FrameType] ) -> bytes:
        ...
        for frame in frames:
            if isinstance( frame, SilentFrame ):
                ...
                writer.write(
                    find_closest_index(
                        cls.ENERGY_LUT[cls.ENERGY_LUT_FLOOR : -1], frame.energy
                    )
                    + cls.ENERGY_LUT_FLOOR,
                    cls.ENERGY_BITS,
                )
                writer.write( 1, cls.REPEAT_BITS )
                writer.write(
                    find_closest_index( cls.PITCH_LUT, frame.pitch ), cls.PITCH_BITS
                )
                ...
                writer.write(
                    find_closest_index( cls.K_LUT[0], frame.k1 ), cls.K_BITS[0]
                )
                writer.write(
                    find_closest_index( cls.K_LUT[1], frame.k2 ), cls.K_BITS[1]
                )
                writer.write(
                    find_closest_index( cls.K_LUT[2], frame.k3 ), cls.K_BITS[2]
                )
                writer.write(
                    find_closest_index( cls.K_LUT[3], frame.k4 ), cls.K_BITS[3]
                )
                ...
                writer.write( 0, cls.REPEAT_BITS )
                writer.write(
                    find_closest_index( cls.PITCH_LUT[1:], frame.pitch ) + 1,
                    cls.PITCH_BITS,
                )
                writer.write(
                    find_closest_index( cls.K_LUT[0], frame.k1 ), cls.K_BITS[0]
                ...
                writer.write(
                    find_closest_index( cls.K_LUT[4], frame.k5 ), cls.K_BITS[4]
                )
                writer.write(
                    find_closest_index( cls.K_LUT[5], frame.k6 ), cls.K_BITS[5]
                )
                writer.write(
                    find_closest_index( cls.K_LUT[6], frame.k7 ), cls.K_BITS[6]
                )
                writer.write(
                    find_closest_index( cls.K_LUT[7], frame.k8 ), cls.K_BITS[7]
                )
                writer.write(
                    find_closest_index( cls.K_LUT[8], frame.k9 ), cls.K_BITS[8]
                )
                writer.write(
                    find_closest_index( cls.K_LUT[9], frame.k10 ), cls.K_BITS[9]
                )
    ...
    def parse_stream( cls, buffer: BytesReadType ) -> TISpeechSample:
        ...
        while not reader.tell() == (len( buffer ), 0):
            ...
            if repeat:
                frames.append(
                    RepeatedFrame(
                        energy=cls.ENERGY_LUT[energy], pitch=cls.PITCH_LUT[pitch]
                    )
                )
                continue
