...
class Palette( mrc.BlockField ):
    def __init__(
        self, block_klass, offset, block_kwargs=None, count=None, fill=None, **kwargs
    ):
    ...
    def scrub( self, value, parent=None ):
        return [
            x if isinstance( x, self.block_klass ) else self.block_klass( x )
            for x in value
        ]
...
class CodecImage( Image ):
    ...
    def get_image( self ):
        ...
        if self.width is not None and image.size[0] != self.width:
            logger.warning(
                "Image {} has width {}, was expecting {}".format(
                    self, image.size[0], self.width
                )
            )
        if self.height is not None and image.size[1] != self.height:
            logger.warning(
                "Image {} has height {}, was expecting {}".format(
                    self, image.size[1], self.height
                )
            )
        if self.format is not None and image.format != self.format:
            logger.warning(
                "Image {} is in {} format, was expecting {} format".format(
                    self, image.format, self.format
                )
            )
        if self.mode is not None and image.mode != self.mode:
            logger.warning(
                "Image {} is mode {}, was expecting mode {}".format(
...
class IndexedImage( Image ):
    ...
    @property
    def repr( self ):
        return "<{}: {} bytes, {}x{}>".format(
            self.__class__.__name__, self.width * self.height, self.width, self.height
        )
...
class Planarizer( mrc.Transform ):
    ...
    def import_data( self, buffer: bytes, parent=None ):
        ...
        assert (
            len( buffer )
            >= frame_offset + (frame_count - 1) * frame_stride + bpp * plane_size
        )
            ...
            for bi, b in enumerate( plane_order ):
                for i in range( plane_size ):
                    ...
                    if row_planar_size is None:
                        ...
                        address = (
                            pointer
                            + (row_planar_size * bpp) * (i // row_planar_size)
                            + row_planar_size * b
                            + (i % row_planar_size)
                        )
        ...
        if frame_count > 1:
            ...
            end_offset = (
                frame_offset + (plane_bits) // 8 + (1 if (plane_bits % 8) else 0)
            )
